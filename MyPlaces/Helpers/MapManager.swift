...
//  MyPlaces
//
//  Created by Арина on 09.06.2020.
...
class MapManager {
        ...
        if CLLocationManager.locationServicesEnabled() {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.showAlert(
                    ...
                    messge: "To enable it go: Settings -> Privacy -> Location Services and turn on"
                )
            }
        }
    }
        ...
        case .denied:
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.showAlert(
                    ...
                    messge: "To give permission Go to: Settings -> MyPlaces -> Location"
                )
            }
            break
        case .notDetermined:
    ...
    func getDirections(for mapView: MKMapView, previousLocation: (CLLocation) -> ()) {
        ...
        guard let location = locationManager.location?.coordinate else {
            showAlert(title: "Error", messge: "current location is not found")
            return
        }
        ...
        guard let request = createDirectionsRequest(from: location) else {
            showAlert(title: "Error", messge: "Destination is not found")
            return
        }
        ...
        directions.calculate { (response, error) in
            ...
            guard let response = response else {
                self.showAlert(title: "Error", messge: "Directions is not available")
                return
            }
    ...
    // Alert
    func showAlert(title: String, messge: String) {
